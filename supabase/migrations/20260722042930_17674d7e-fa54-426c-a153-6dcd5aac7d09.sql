...
BEGIN
  ...
  _bypass := current_setting('boardroom.allow_spend_update', true);
  IF COALESCE(_bypass, '') = 'on' THEN
    -- Atomic accounting function: only spent_usd + budget_warning may move.
    ...
    THEN
      RAISE EXCEPTION 'allow_spend_update may only change spent_usd/budget_warning';
    END IF;
    RETURN NEW;
  END IF;
...
BEGIN
  ...
  IF p_run_id IS NOT NULL THEN
    ...
    PERFORM set_config('boardroom.allow_spend_update', 'on', true);
    UPDATE public.boardroom_runs
       SET spent_usd = v_new_total,
           ...
           END
     WHERE id = p_run_id;
    PERFORM set_config('boardroom.allow_spend_update', 'off', true);
    RETURN v_new_total;
  END IF;
