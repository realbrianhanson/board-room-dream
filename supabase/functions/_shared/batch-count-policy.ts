...
export function productStrategyContract(): string {
  return `The document MUST contain a "## Product strategy" H2 section (place it after the concept/user sections, before Data model) with these concrete decisions in bullet form — one bullet per point. For imported apps the owner's supplied intake answers are AUTHORITY: use them verbatim and NEVER silently replace them. Where an owner input is missing, do NOT invent or assume the missing value and do NOT emit a generic "owner-unknown assumption" directive — instead follow the per-field rules below (aligned with supabase/functions/_shared/import-contract.ts):
