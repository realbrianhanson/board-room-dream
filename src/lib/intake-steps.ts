...
export function canProceedFromIntakeStep(kind: IntakeStepKind, a: IntakeAnswers): boolean {
  switch (kind) {
    ...
    case "money":
      // price_anchor AND upgrade_trigger are deliberately optional. An
      // owner may leave either unset when they want the Board to
      // recommend a starting price or upgrade trigger. Blank pricing /
      // trigger here is intentional, not invalid — the App Blueprint
      // never invents a monetization answer for the customer's product.
      return (
        !!a.money &&
